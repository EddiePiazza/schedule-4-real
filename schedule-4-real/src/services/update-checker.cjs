...
/**
 * Probe a TCP port (host:port). Resolves true if something is accepting
 * connections within `timeoutMs`, false otherwise.
 */
...
function isPortHeldByMosquitto(port) {
  try {
    const out = execSync(`ss -tlnp 2>/dev/null | grep ":${port} " | head -1`, { stdio: ['ignore', 'pipe', 'ignore'], timeout: 3000 }).toString();
    return /mosquitto/.test(out);
  } catch { return false; }
}

/**
 * Validate the mosquitto config by booting it in foreground for ~3s. Returns
 * { ok, stderr }. Catches the most common failures so we can react:
 *   - "Address already in use" → another mosquitto/system service holds 1883
 *   - "Unable to open ... mosquitto.db" → corrupt persistence
 *   - "Error: Unable to open log file" → bad path / perms
 */
async function validateMosquittoConfig(confPath) {
  return new Promise((resolve) => {
    const proc = require('child_process').spawn('mosquitto', ['-c', confPath, '-v'], { stdio: ['ignore', 'pipe', 'pipe'] });
    let stderr = '';
    proc.stderr.on('data', d => { stderr += d.toString(); });
    const t = setTimeout(() => {
      try { proc.kill('SIGTERM'); } catch {}
      resolve({ ok: !stderr.match(/error|fail|unable|cannot|invalid/i), stderr });
    }, 2500);
    proc.on('error', (err) => { clearTimeout(t); resolve({ ok: false, stderr: stderr || err.message }); });
    proc.on('exit', (code) => {
      clearTimeout(t);
      // Exit 3 = config error, 1 = bind error, 0 = clean shutdown
...
/**
 * Regenerate proxy/mosquitto.conf from .env settings — the same template
 * pm2-start.sh writes. Used when the file is missing or syntactically broken.
 */
function writeMosquittoConfig() {
...
function killSystemMosquitto() {
  try {
    execSync('systemctl is-active --quiet mosquitto.service', { stdio: 'ignore' });
    console.log('[Recovery] Stopping & disabling system mosquitto.service');
...
function killOrphanMosquittos() {
  ...
  // launched with system/etc paths.
  try {
    execSync(`pkill -f 'mosquitto -c /etc/'`, { stdio: 'pipe' });
  } catch {}
  // Also kill anything launched detached by the legacy daemon-mode restart
  try {
    const out = execSync(`pgrep -af mosquitto || true`, { stdio: ['ignore', 'pipe', 'ignore'] }).toString();
    const lines = out.split('\n').filter(Boolean);
    for (const line of lines) {
      // Skip our project's instance (started by PM2, has our config path)
      if (line.includes(`${PROJECT_ROOT}/proxy/mosquitto.conf`)) continue;
...
async function recreateMosquittoPm2(name) {
  const cfg = path.join(PROJECT_ROOT, 'proxy', 'mosquitto.conf');
  // Remove stale entry (if any) so PM2's restart counter starts clean
  try { execSync(`pm2 delete ${name}`, { stdio: 'pipe' }); } catch {}
  // Recreate. --interpreter none keeps PM2 from trying to run mosquitto via node
  // on certain PM2 versions that misdetect non-extension binaries.
  const cmd = [
    'pm2', 'start', 'mosquitto',
    '--name', name,
    '--interpreter', 'none',
  ...
  try {
    execSync(cmd, { stdio: 'pipe', timeout: 10000, cwd: PROJECT_ROOT });
  } catch (err) {
    console.error(`[Recovery] Failed to (re)start ${name} via PM2:`, err.message);
  }
}

/**
 * Make sure the local Mosquitto broker is reachable on 127.0.0.1:${MQTT_PORT}.
 * Runs end-to-end remediation (system mosquitto, orphans, config, persistence,
 * PM2 entry) regardless of what PM2 thinks the status is — the only thing that
 * matters is whether something is actually accepting connections on the port.
 *
 * This addresses both Christian's "ECONNREFUSED 127.0.0.1:1883" report and
 * JoeGhost's "errored even after reboot, only fixed by reinstalling" report.
 */
async function ensureMosquittoHealthy() {
  const port = parseInt(process.env.MQTT_PORT) || 1883;
  if (await probePort('127.0.0.1', port, 1200)) return; // healthy, nothing to do

  console.log(`[Recovery] Mosquitto not responding on 127.0.0.1:${port} — running remediation`);
  ...
  // 3) Make sure config file exists and is sane (if path moved or got wiped, regenerate)
  const confPath = path.join(PROJECT_ROOT, 'proxy', 'mosquitto.conf');
  if (!fs.existsSync(confPath)) {
    console.log('[Recovery] mosquitto.conf missing — regenerating from .env');
    writeMosquittoConfig();
  }

  // 4) Validate config; rotate corrupt persistence DB if that's the cause
  let validation = await validateMosquittoConfig(confPath);
  if (!validation.ok) {
  ...
  if (!validation.ok) {
    ...
    if (stderrLower.includes('persistence') || stderrLower.includes('mosquitto.db') || stderrLower.includes('unable to open')) {
      ...
      const ts = Date.now();
      try {
        fs.renameSync(dataDir, `${dataDir}.broken-${ts}`);
        fs.mkdirSync(dataDir, { recursive: true });
        console.log(`[Recovery] Rotated corrupt persistence dir to mosquitto_data.broken-${ts}`);
      } catch (e) {
        console.error(`[Recovery] Could not rotate persistence dir:`, e.message);
      }
    } else if (stderrLower.includes('unable to find') && stderrLower.includes('mosquitto')) {
      // Binary missing — try to install via apt
      try {
        console.log('[Recovery] mosquitto binary missing — attempting apt install');
        execSync('apt-get update && apt-get install -y mosquitto', { stdio: 'pipe', timeout: 120000 });
      } catch (e) {
        console.error(`[Recovery] apt install failed (binary still missing):`, e.message);
      }
    } else {
      // Bad config we can't auto-fix — regenerate the whole thing from .env
      console.log('[Recovery] Regenerating mosquitto.conf from scratch (config looked broken)');
      writeMosquittoConfig();
    }
    // Re-validate after fixes
    validation = await validateMosquittoConfig(confPath);
    if (!validation.ok) {
      console.warn('[Recovery] Mosquitto config still invalid after fixes — leaving for manual intervention');
      console.warn(validation.stderr.slice(0, 600));
    }
  }

  // 5) Recreate PM2 entry from scratch so any stuck-errored state is wiped
  const list = getPm2List();
  const existing = list && list.find(p => p.name === 's4r-mosquitto' || p.name === 'spiderapp-mosquitto');
  const name = existing ? existing.name : 's4r-mosquitto';
  await recreateMosquittoPm2(name);

  // 6) Wait up to 10s for port to come up
  for (let i = 0; i < 10; i++) {
    await new Promise(r => setTimeout(r, 1000));
    if (await probePort('127.0.0.1', port, 800)) {
      console.log(`[Recovery] Mosquitto recovered — listening on 127.0.0.1:${port}`);
      try { execSync('pm2 save', { stdio: 'pipe', timeout: 5000 }); } catch {}
      return;
    }
  }

  // Still down — collect diagnostics for the next investigation
  let diag = '(no diagnostics)';
  try {
    diag = execSync(`tail -n 20 ${path.join(PROJECT_ROOT, 'logs/mosquitto-error.log')} 2>/dev/null; echo ---; tail -n 20 ${path.join(PROJECT_ROOT, 'logs/mosquitto.log')} 2>/dev/null`,
      { stdio: ['ignore', 'pipe', 'ignore'], timeout: 3000 }).toString();
  } catch {}
  console.error(`[Recovery] Mosquitto still down on :${port} after remediation. Diagnostics:\n${diag}`);
}

/**
 * Same idea as ensureMosquittoHealthy but for the TLS proxy on :8883. The proxy
...
async function ensureProxyHealthy() {
  ...
  if (existing) {
    ...
    if (fs.existsSync(binary)) {
      try {
        execSync(`pm2 start ${binary} --name s4r-proxy --interpreter none --cwd ${PROJECT_ROOT} --log ./logs/proxy.log --error ./logs/proxy-error.log`,
          { stdio: 'pipe', timeout: 10000, cwd: PROJECT_ROOT });
      } catch (err) {
        console.error(`[Recovery] Could not (re)start s4r-proxy:`, err.message);
      }
    }
  }
  ...
  console.error(`[Recovery] Proxy still down on :${port} after remediation`);
}

/**
 * Periodic supervisor entry-point. Cheap when everything is healthy (one TCP
 * probe per port) and only does the heavy lifting when a service is actually
 * down. Replaces the older `recoverErroredServices` which only fired on PM2's
 * "errored" status — useless when the entry is missing, "online" but dead, or
 * stuck because of a port conflict that PM2 can't see.
 */
async function recoverErroredServices() {
  try {
    await ensureMosquittoHealthy();
    await ensureProxyHealthy();
  } catch (err) {
    console.error('[Recovery] Service recovery error:', err.message);
  }
}
