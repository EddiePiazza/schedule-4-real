...
# Auto-generated by installer (pm2-start.sh regenerates this on every boot — keep in sync).
# Persistence is OFF on purpose: this is a transient relay broker (the real data lives in QuestDB).
# A persistence DB write under a relative path resolved against a root-owned working dir throws
# "mosquitto.db.new: Permission denied" and kills the broker on boot; persistence false avoids it.
# allow_anonymous is GLOBAL (we do NOT set per_listener_settings), so it applies to every listener.
